...
Settings.prototype.adminSecurityHash = function(originalPassword) {
  return bcrypt.hashSync(originalPassword, this.customSalt);
}

Settings.prototype.adminSectionEnabled = function() {
    ...
    fs.readFile(folderPath+'/config.json', function(error, fileData) {
      ...
      if(error) {
        ...
        try {
          ...
          if(settingsParsed.adminPassword) {
            thisSettings.adminPassword = thisSettings.adminSecurityHash(settingsParsed.adminPassword);
          }
