...
distribution.appList = function(callback) {
    ...
    fs.readdir(path, function(err, files) {
        if (err) {
            ...
            var filesFiltered = _.reject(files, function(file) {
                return file == "config.json" || file.indexOf(".") === 0 || !distribution.appExistSync(file)
            });
...
distribution.versionList = function(app, callback) {
    ...
    fs.readdir(path, function(err, files) {
        ...
        if (err) {
            ...
            var filesFiltered = _.reject(files, function(file) {
                return file == "config.json" || file.indexOf(".") === 0
            });
            filesFiltered = _.filter(files, function(file) {
            ...
            var statFile = function() {
                ...
                if (file) {
                    ...
                    fs.stat(filePath, function(err, stat) {
                        ...
                        detail[os] = {
                            size: (stat.size / 1024 / 1024),
                            creationDate: stat.birthtime || stat.mtime,
                            lastChangeDate: stat.mtime
                        };
...
distribution.createApp = function(appName, appImageData, configuration, callback) {

    distribution.appExist(appName, function(exist) {
        if (exist) {
        ...
        fs.mkdir(path, function(err) {
            ...
            var createConfiguration = function() {
                var configFileData = JSON.stringify(configuration, null, '\t');
                fs.writeFile(path + "/config.json", configFileData, function(err) {
                    if (err) {
                        fs.rmdirSync(path);
                        return callback(false);
                    }
            ...
            if (appImageData) {
                fs.writeFile(path + "/logo", appImageData, function(err) {
                    if (err) {
                        fs.rmdirSync(path);
                        return callback(false);
                    }

                    configuration.imageFile = "logo";
...
distribution.removeApp = function(appName, callback) {
